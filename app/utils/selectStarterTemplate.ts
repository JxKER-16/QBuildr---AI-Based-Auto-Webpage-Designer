...
export async function getTemplates(templateName: string, title?: string) {
  ...
  // exclude    .qbuildr
  filteredFiles = filteredFiles.filter((x) => x.path.startsWith('.qbuildr') == false);

  // check for ignore file in .qbuildr folder
  const templateIgnoreFile = files.find((x) => x.path.startsWith('.qbuildr') && x.name == 'ignore');
...
<qbuildrArtifact id="imported-files" title="${title || 'Importing Starter Files'}" type="bundled">
${filesToImport.files
  .map(
    (file) =>
...
`;
  let userMessage = ``;
  const templatePromptFile = files.filter((x) => x.path.startsWith('.qbuildr')).find((x) => x.name == 'prompt');
