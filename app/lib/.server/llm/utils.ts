...
export function simplifyqbuildrActions(input: string): string {
  // Using regex to match qbuildrAction tags that have type="file"
  const regex = /(<qbuildrAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/qbuildrAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<qbuildrAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</qbuildrAction>`;
    });

  return `<qbuildrArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</qbuildrArtifact>`;
}
