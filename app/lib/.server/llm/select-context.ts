...
import { createFilesContext, extractCurrentContext, extractPropertiesFromMessage, simplifyqbuildrActions } from './utils';
...
export async function selectContext(props: {
  ...
  const processedMessages = messages.map((message) => {
    if (message.role === 'user') {
      ...
      content = simplifyqbuildrActions(content);

      content = content.replace(/<div class=\\"__qbuildrThought__\\">.*?<\/div>/s, '');
